...
export class Path extends FakeSVG
{
    ...
    semiArc(from: EightCardinals, spin: Spin)
    {
        ...
        if (spin == 'Clockwise') {
            switch(from) {
                case('North'): return `${path} ${section} ${inversed}`;
                case('North East'): return `${path} ${inversed} ${section}`;
                case('East'): return `${path} -${inversed} ${section}`;
                case('South East'): return `${path} -${section} ${inversed}`;
                case('South'): return `${path} -${section} -${inversed}`;
                case('South West'): return `${path} -${inversed} -${section}`;
                case('West'): return `${path} ${inversed} -${section}`;
                case('North West'): return `${path} ${section} -${inversed}`;
            }
        }
        ...
        if (spin == 'Counter clockwise') {
            switch(from) {
                case('North'): return `${path} -${section} ${inversed}`;
                case('North West'): return `${path} -${inversed} ${section}`;
                case('West'): return `${path} ${inversed} ${section}`;
                case('South West'): return `${path} ${section} ${inversed}`;
                case('South'): return `${path} ${section} -${inversed}`;
                case('South East'): return `${path} ${inversed} -${section}`;
                case('East'): return `${path} -${inversed} -${section}`;
                case('North East'): return `${path} -${section} -${inversed}`;
            }
        }

        throw new Error('Unexpected values for a semi arc');
    }
}